...
export const authService = {
    ...
    async login({ email, password }, ip, device) {
        ...
        if (!await comparePassword(password, user.password)) throw { status: 401, message: "Sai mật khẩu" };
    ...
    async resetPassword({ email, otpCode, newPassword }, ip, device) {
        ...
        if (!await otpService.verify(email, otpCode)) throw { status: 400, message: "Mã OTP không hợp lệ" };

        newPassword = await hashPassword(newPassword);
        await user.update({ password: newPassword });

        await RefreshToken.update({ revokedAt: new Date(), revokedByIp: ip }, { where: { userId: user.id, revokedAt: null } });
    ...
    async updatePassword(userId, currentPassword, newPassword) {
        ...
        if (!await comparePassword(currentPassword, user.password)) throw { status: 401, message: "Sai mật khẩu hiện tại" };
    ...
    async sendOtp(email, type) {
        ...
        await sendMail(email, type, { otp, otpExpiresInMinutes: env.OTP_EXPIRE_MINUTES });
    },
